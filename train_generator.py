...
def get_opt():
    ...
    parser.add_argument('--tocg_checkpoint', type=str,
                        help='condition generator checkpoint')
    parser.add_argument('--gen_checkpoint', type=str,
                        default='', help='gen checkpoint')
    parser.add_argument('--dis_checkpoint', type=str,
                        default='', help='dis checkpoint')
    ...
    parser.add_argument('--GMM_const', type=float,
                        default=None, help='constraint for GMM module')
...
def train(opt, train_loader, test_loader, test_vis_loader, board, tocg, generator, discriminator, model):
    ...
    optimizer_gen = torch.optim.Adam(
        generator.parameters(), lr=opt.G_lr, betas=(0, 0.9))
    ...
    optimizer_dis = torch.optim.Adam(
        discriminator.parameters(), lr=opt.D_lr, betas=(0, 0.9))
    ...
    if len(opt.gpu_ids) > 0:
        ...
        discriminator = DataParallelWithCallback(
            discriminator, device_ids=opt.gpu_ids)
        criterionGAN = DataParallelWithCallback(
            criterionGAN, device_ids=opt.gpu_ids)
        criterionFeat = DataParallelWithCallback(
            criterionFeat, device_ids=opt.gpu_ids)
        criterionVGG = DataParallelWithCallback(
            criterionVGG, device_ids=opt.gpu_ids)
    ...
    for step in tqdm(range(opt.load_step, opt.keep_step + opt.decay_step)):
        ...
        with torch.no_grad():
            if not opt.GT:
                ...
                warped_cloth_paired = F.grid_sample(
                    c_paired, warped_grid, padding_mode='border').detach()
                warped_clothmask = F.grid_sample(
                    cm, warped_grid, padding_mode='border')

                # make generator input parse map
                fake_parse_gauss = gauss(F.interpolate(
                    fake_segmap, size=(iH, iW), mode='bilinear'))
                ...
                if opt.occlusion:
                    ...
                    warped_cloth_paired = warped_cloth_paired * warped_clothmask + \
        ...
        #                                              Train the generator
        # --------------------------------------------------------------------------------------------------------------
        output_paired = generator(
            torch.cat((agnostic, pose, warped_cloth_paired), dim=1), parse)
        ...
        if type(pred) == list:
            ...
            pred_real = []
            for p in pred:
                pred_fake.append([tensor[:tensor.size(0) // 2]
                                 for tensor in p])
                pred_real.append([tensor[tensor.size(0) // 2:]
                                 for tensor in p])
        else:
        ...
        D_losses = {}
        D_losses['D_Fake'] = criterionGAN(
            pred_fake, False, for_discriminator=True)
        D_losses['D_Real'] = criterionGAN(
        ...
        if (step + 1) % opt.tensorboard_count == 0:
            ...
            grid = make_image_grid([(c_paired[0].cpu() / 2 + 0.5), (cm[0].cpu()).expand(3, -1, -1), ((pose.cpu()[0]+1)/2), visualize_segmap(parse_agnostic.cpu(), batch=i),
                                    (warped_cloth_paired[i].cpu() / 2 + 0.5), (agnostic[i].cpu() / 2 + 0.5), (pose[i].cpu(
                                    ) / 2 + 0.5), visualize_segmap(fake_parse_gauss.cpu(), batch=i),
            ...
            board.add_scalar(
                'Loss/gen/adv', G_losses['GAN'].mean().item(), step + 1)
            # board.add_scalar('Loss/gen/l1', G_losses['L1'].mean().item(), step + 1)
            board.add_scalar(
                'Loss/gen/feat', G_losses['GAN_Feat'].mean().item(), step + 1)
            board.add_scalar(
                'Loss/gen/vgg', G_losses['VGG'].mean().item(), step + 1)
            board.add_scalar('Loss/dis', loss_dis.item(), step + 1)
            board.add_scalar('Loss/dis/adv_fake',
                             D_losses['D_Fake'].mean().item(), step + 1)
            board.add_scalar('Loss/dis/adv_real',
                             D_losses['D_Real'].mean().item(), step + 1)
                ...
                for i in tqdm(range(500)):
                    ...
                    with torch.no_grad():
                        if not opt.GT:
                            ...
                            fake_parse_gauss = gauss(F.interpolate(
                                fake_segmap, size=(iH, iW), mode='bilinear'))
                            fake_parse = fake_parse_gauss.argmax(dim=1)[
                                :, None]
        ...
        if (step + 1) % opt.save_count == 0:
            save_checkpoint(generator.module, os.path.join(
                opt.checkpoint_dir, opt.name, 'gen_step_%06d.pth' % (step + 1)), opt)
            save_checkpoint(discriminator.module, os.path.join(
                opt.checkpoint_dir, opt.name, 'dis_step_%06d.pth' % (step + 1)), opt)
...
def main():
    ...
    # Train
    train(opt, train_loader, test_loader, test_vis_loader,
          board, tocg, generator, discriminator, model)

    # Save Checkpoint
    save_checkpoint(generator, os.path.join(
        opt.checkpoint_dir, opt.name, 'gen_model_final.pth'), opt)
    save_checkpoint(discriminator, os.path.join(
        opt.checkpoint_dir, opt.name, 'dis_model_final.pth'), opt)
