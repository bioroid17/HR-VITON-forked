...
def get_opt():
    ...
    parser.add_argument('--D_checkpoint', type=str,
                        default='', help='D checkpoint')
    ...
    parser.add_argument('--norm_const', type=float,
                        help='Normalizing constant for rejection sampling')
        ...
        with torch.no_grad():
            ...
            if D is not None:
                ...
                for i in range(cm_paired.shape[0]):
                    name = inputs['c_name']['paired'][i].replace(
                        '.jpg', '.png')
...
def main():
    ...
    board = SummaryWriter(log_dir=os.path.join(opt.tensorboard_dir, opt.tocg_checkpoint.split(
        '/')[-2], opt.tocg_checkpoint.split('/')[-1], opt.datamode, opt.datasetting))
